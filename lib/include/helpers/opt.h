...
/*
  ...
  Y	log_2(init_size)
*/
#define X_TO_SIZE(X, Y) (1 << ((X) + (Y) + ((X) == 0)))

/*
  X	index value
  Y	log_2(init_size)
*/
#define IDX_TO_X(X, Y) fl1_32((X) >> (Y))
...
#define ff0_32_ffs(X) ff1_32_P1(~(X))

// ff -> find first, i.e ff1 of 0b10101000 would return 3
#define ff1_64_ffs(X) __builtin_ffsll((X))
#define ff0_64_ffs(X) ff1_64_P1(~(X))
