...
#define high_bits_set_DECR(X) highBitsSetDECR((void **)(&(X)))
void highBitsSetDECR(void ** n);

#define high_bits_set_INCR(X) highBitsSetINCR((void **)(&(X)))
...
#define set_ptr_atomic(X, Y, Z)                                                \
    setPtr_atomic((void **)(&(X)), (void *)(Y), (void *)(Z))
uint32_t setPtr_atomic(void ** n, void * expec, void * new_ptr);

#define low_bits_set_atomic(X, Y, Z)                                           \
    lowBitsSet_atomic((void **)(&(X)), (void *)(Y), (Z))
...
#define low_bits_set_AND_atomic(X, Y) lowBitsSetAND_atomic((void **)(&(X)), (Y))
uint64_t lowBitsSetAND_atomic(void ** n, uint64_t bits);

#define low_bits_set_XOR_atomic(X, Y) lowBitsSetXOR_atomic((void **)(&(X)), (Y))
uint64_t lowBitsSetXOR_atomic(void ** n, uint64_t bits);

#define low_bits_set_OR_atomic(X, Y) lowBitsSetOR_atomic((void **)(&(X)), (Y))
uint64_t lowBitsSetOR_atomic(void ** n, uint64_t bits);


#define high_bits_set_atomic(X, Y, Z)                                          \
    highBitsSet_atomic((void **)(&(X)), (void *)(Y), (Z))
uint32_t highBitsSet_atomic(void ** n, void * expec, uint16_t bits);

#define high_bits_set_ADD_atomic(X, Y)                                         \
    highBitsSetADD_atomic((void **)(&(X)), (Y))
