...
getCacheLineSize() {
    FILE * fp =
        myfopen("/sys/devices/system/cpu/cpu0/cache/index0/coherency_line_size",
                "r");
    char buf[16] = "";
    if (!fgets(buf, 8, fp)) {
...
main(int32_t argc, char * argv[]) {
    ...
    if ((result = createTempBasePath(transfer_path)) == NOT_FOUND) {
        errdie(
            "Couldn't find directory with core temp info. Please check temp.h "
            "params\n");
    }
    ...
    for (int32_t i = 0; i < BASELINE_LINES; i++) {
        ...
        if (!strncmp(file_constants[i],
            ...
            fprintf(config_fp, "%s (%dUL)\n", file_constants[i], ncores);
                          ...
                          "#define PAGE_SIZE",
                          strlen("#define PAGE_SIZE"))) {
            fprintf(config_fp, "%s (%dUL)\n", file_constants[i], page_size);
        }
